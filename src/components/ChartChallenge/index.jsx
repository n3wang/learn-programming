...
/**
 * Same target-vs-yours pattern as GraphChallenge, but for bar/pie charts:
 * the student writes a function that returns a list of numbers (one per
 * category) instead of a y=f(x) function sampled over a range.
 */
export default function ChartChallenge({
  ...
  const harnessOpts = useMemo(
    () => ({kind, functionName, categories, targetValues, ylabel, unit}),
    [kind, functionName, categories, targetValues, ylabel, unit]
  );

  const harness = useMemo(() => buildHarness(harnessOpts), [harnessOpts]);

  useEffect(() => {
    mounted.current = true;
    const targetHarness = buildTargetOnlyHarness(harnessOpts);
  ...
  return (
    <div className={chrome.shell}>
      ...
      <div className={chrome.editorPane}>
        <CodeEditor value={code} onChange={setCode} lang={lang} height="140px" />
      </div>

...
/**
 * Lightweight SVG charts — bars, line, or simple heatmap. No chart libraries.
 *
...
export default function FormulaChart({
  ...
  const layout = useMemo(() => {
    ...
    const yMax = Math.max(1e-9, ...ys) * 1.08;
    const xs = series.map((p) => p.x);
    const xMin = xs.length ? Math.min(...xs) : 0;
    const xMax = xs.length ? Math.max(...xs) : 1;
    const xSpan = xMax - xMin || 1;
    return {innerW, innerH, yMax, xMin, xMax, xSpan};
  ...
  const {innerW, innerH, yMax, xMin, xSpan} = layout;

...
async function isPistonReachable(executeUrl, timeoutMs = 750) {
  const runtimesUrl = executeUrl.replace(/\/execute\/?$/, '/runtimes');
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const res = await fetch(runtimesUrl, {signal: controller.signal});
    return res.ok;
  } catch {
    return false;
  } finally {
...
export default async function createConfig() {
  ...
  // Dev → local Spring Boot. Production builds MUST set API_BASE_URL (CI / host env).
  const defaultApiBaseUrl = 'http://localhost:8080';
  const apiBaseUrl = (process.env.API_BASE_URL || defaultApiBaseUrl).replace(
    /\/$/,
    '',
  );
  if (!isDevServer && !process.env.API_BASE_URL) {
    console.warn(
      '[docusaurus] API_BASE_URL is unset — production clients will call localhost:8080. Set API_BASE_URL to your springbackend origin (https://…).',
    );
  }

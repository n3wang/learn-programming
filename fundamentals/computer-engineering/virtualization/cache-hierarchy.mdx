...
<MultipleChoice
  id="ce-cache-hierarchy-order-levels"
  title="Rank the sequence"
  questions={[
    {
      type: 'order',
      prompt: 'Order the memory hierarchy from fastest/smallest to slowest/largest.',
      items: ['L3 Cache', 'Registers', 'DRAM', 'L1 Cache', 'L2 Cache'],
      order: [1, 3, 4, 0, 2],
      why: 'Registers → L1 → L2 → L3 → DRAM: each step trades speed for capacity as you move away from the CPU core.',
    },
    {
      type: 'order',
      prompt: 'Order what happens on a cache miss (top = first).',
      items: [
        'Retry the original access, which now hits',
        'CPU requests an address; it is not found in the cache',
        'The fetched line is placed into the cache (a fill)',
        'The next level down is queried for that address\'s cache line',
      ],
      order: [1, 3, 2, 0],
      why: 'A miss triggers a fetch from the next level, the returned line fills the cache, and only then is the original request retried and served.',
    },
  ]}
/>
